...
pidgin_dialogs_ee(const char *ee)
{
	...
	window = gtk_dialog_new_with_buttons(PIDGIN_ALERT_TITLE, NULL, 0, pidgin_stock_label(GTK_STOCK_CLOSE), GTK_RESPONSE_OK, NULL);
	...
	gtk_box_set_spacing(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(window))), PIDGIN_HIG_BORDER);
	gtk_container_set_border_width (GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(window))), PIDGIN_HIG_BOX_SPACE);
	...
	img = pidgin_image_new_from_stock(PIDGIN_STOCK_DIALOG_COOL, gtk_icon_size_from_name(PIDGIN_ICON_SIZE_TANGO_HUGE));
