...
static void pidgin_status_box_get_preferred_width (GtkWidget *widget, gint *minimal, gint *natural);
static void pidgin_status_box_get_preferred_height (GtkWidget *widget, gint *minimal, gint *natural);
                              ...
                              G_GNUC_UNUSED gpointer class_data)
{
	...
	widget_class->get_preferred_width = pidgin_status_box_get_preferred_width;
					...
					gconstpointer value, gpointer data)
{
	...
	else
	{
		...
		spell = gtk_spell_checker_get_from_text_view(GTK_TEXT_VIEW(status_box->imhtml));
		if (spell)
  ...
  *width = allocation.width;
  ...
  hpolicy = vpolicy = GTK_POLICY_NEVER;
	g_object_set(G_OBJECT(status_box->scrolled_window),
		...
		NULL);
  gtk_widget_get_preferred_size_compat(status_box->popup_frame, &popup_req);
...
static void
pidgin_status_box_get_requisition(GtkWidget *widget, GtkRequisition *requisition)
{
	GtkRequisition box_req;
	guint border_width = gtk_container_get_border_width(GTK_CONTAINER(widget));

	gtk_widget_get_preferred_size_compat(PIDGIN_STATUS_BOX(widget)->toggle_button, requisition);
...
static void
pidgin_status_box_get_preferred_width(GtkWidget *widget,
				      gint *minimal, gint *natural)
{
	GtkRequisition requisition;
	pidgin_status_box_get_requisition(widget, &requisition);
	*minimal = *natural = requisition.width;
}

static void
pidgin_status_box_get_preferred_height(GtkWidget *widget,
				       gint *minimal, gint *natural)
{
	GtkRequisition requisition;
	pidgin_status_box_get_requisition(widget, &requisition);
				  ...
				  GtkAllocation *allocation)
{
	...
	gtk_widget_get_preferred_size_compat(status_box->toggle_button, &req);
	/* Make this icon the same size as other buddy icons in the list; unless it already wants to be bigger */
...
pidgin_status_box_draw(GtkWidget *widget, cairo_t *cr)
{
	...
	if (status_box->icon_box && status_box->icon_opaque) {
		...
		gtk_widget_get_allocation(status_box->icon_box, &icon_alc);
		gtk_style_context_save(context);
		gtk_style_context_add_class(context, GTK_STYLE_CLASS_BUTTON);
		gtk_render_frame(context, cr, icon_alc.x-1, icon_alc.y-1, 34, 34);
		gtk_render_background(context, cr, icon_alc.x-1, icon_alc.y-1, 34, 34);
