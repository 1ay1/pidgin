					       ...
					       GtkCellRendererState flags)
{
	...
	gtk_style_context_set_state(context,
		state | (is_expanded ? GTK_STATE_FLAG_CHECKED : 0));

	{
		gint expander_size = 0;
		gdouble ex, ey;
		gtk_widget_style_get(widget, "expander-size", &expander_size, NULL);
		ex = cell_area->x + xpad + (width - expander_size) / 2.0;
		ey = cell_area->y + ypad + (height - expander_size) / 2.0;
		gtk_render_expander(context, cr, ex, ey, expander_size, expander_size);
	}

	gtk_style_context_restore(context);

	/* only draw the line if the color isn't set - this prevents a bug where
	 * the hline appears only under the expander */
	g_object_get(cellexpander, "cell-background-set", &set, NULL);
	if (is_expanded && !set) {
		GtkAllocation alloc;
		gtk_widget_get_allocation(widget, &alloc);
		gtk_render_line(context, cr,
		                0, cell_area->y + cell_area->height,
		                alloc.width, cell_area->y + cell_area->height);
	}
}
