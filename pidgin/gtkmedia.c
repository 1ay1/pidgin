...
phone_create_button(const gchar *text_hi, const gchar *text_lo)
{
	...
	pidgin_widget_set_alignment(GTK_WIDGET(label_hi), 0.5, 0.5);
	gtk_box_pack_end(GTK_BOX(grid), label_hi, FALSE, TRUE, 0);
	label_lo = gtk_label_new(text_lo);
	pidgin_widget_set_alignment(GTK_WIDGET(label_lo), 0.5, 0.5);
...
pidgin_media_ready_cb(PurpleMedia *media, PidginMedia *gtkmedia, const gchar *sid)
{
	...
	if (type & PURPLE_MEDIA_RECV_VIDEO) {
		...
		pidgin_widget_set_bg_color(remote_video, &color);
	...
	if (type & PURPLE_MEDIA_SEND_VIDEO && !gtkmedia->priv->local_video) {
		...
		pidgin_widget_set_bg_color(local_video, &color);
	...
	if (type & PURPLE_MEDIA_VIDEO) {
		icon = pidgin_pixbuf_from_stock(PIDGIN_STOCK_TOOLBAR_VIDEO_CALL, 48);
	} else if (type & PURPLE_MEDIA_AUDIO) {
		icon = pidgin_pixbuf_from_stock(PIDGIN_STOCK_TOOLBAR_AUDIO_CALL, 48);
	}
